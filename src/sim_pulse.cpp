...
DataFrame sim_pulse(int n, arma::vec stimulus, double v, double a, double t0,
                      ...
                      double dt=.001, bool use_weibull_bound=false, int n_threads=1){
...
#pragma omp parallel for
  for (int i=0; i<n_threads; i++){
    ...
    while((n_drift > 0) & (step < stimulus.n_elem)){
      v_var = v + sqrt(sv)*arma::randn(n_drift);
      x(still_drift) += sqrt(s*dt)*arma::randn(n_drift) + stimulus[step]*v_var*dt + lambda*x(still_drift)*dt;
      rt(still_drift) += dt;
