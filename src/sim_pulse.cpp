...
List sim_pulse(int n, arma::cube stimuli, double v, double a, double t0,
  ...
  n_on_thread.fill(n_total / n_threads);
...
#pragma omp parallel for num_threads(n_threads)
  for (int i=0; i<n_threads; i++){
    ...
    arma::mat accumulators(n_on_thread(i), stimuli.n_cols+1);
    if (return_accu) {
...
// //' @export
// // [[Rcpp::export]]
// List sim_pulse_vec(int n, List stimuli, arma::vec a, arma::vec t0,
...
//   
//   if (check_pars) {
//     arma::uvec lens = {stimuli.length(), a.n_elem, t0.n_elem, z.n_elem,
...
//       v_var(still_drift) = v_scale + sqrt(sv(i)) * arma::randn(n_drift);
